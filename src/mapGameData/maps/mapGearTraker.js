...
const findRecipe = ( recipeId, lang, equipmentList, recipeList, components = {}, salvage = {}) => {
  ...
  for(let i in tempRecipe.ingredients){
    ...
    if(tempEquip?.recipeId){
      components[tempEquip.id] = { id: tempEquip.id, nameKey: lang[tempEquip.nameKey] || tempEquip.nameKey, iconKey: tempEquip.iconKey, recipeId: tempEquip.recipeId }
      findRecipe(tempEquip?.recipeId, lang, equipmentList, recipeList, components, salvage)
    }else{
      salvage[tempEquip.id] = { id: tempEquip.id, nameKey: lang[tempEquip.nameKey] || tempEquip.nameKey, iconKey: tempEquip.iconKey, recipeId: tempEquip.recipeId }
    }
  }
}
const findRelicMats = (ingredients = [], materialList = [], lang, relicSalvage = {}, relicMat = {})=>{
  ...
  for(let i in ingredients){
    ...
    if(tempMat.type == 11) relicMat[tempMat.id] = { id: tempMat.id, nameKey: lang[tempMat.nameKey] || tempMat.nameKey, iconKey: tempMat.iconKey }
    if(tempMat.type == 12) relicSalvage[tempMat.id] = { id: tempMat.id, nameKey: lang[tempMat.nameKey] || tempMat.nameKey, iconKey: tempMat.iconKey }
  }
}
module.exports = async( gameVersion, localeVersion) =>{
  ...
  for(let i in equipment){
    ...
    equip[equipment[i].id] = { id: equipment[i].id, nameKey: lang[equipment[i].nameKey] || equipment[i].nameKey, iconKey: equipment[i].iconKey, recipeId: equipment[i].recipeId }
    if(!equipment[i].recipeId) continue;
    findRecipe(equipment[i].recipeId, lang, equipmentList, recipeList, components, salvage)
  }
  await mongo.set('configMaps', { _id: 'gearDefMap' }, { gear: equip, components: components, salvage: salvage, relicMat: relicMat, relicSalvage: relicSalvage, abilityMat: abilityMat })
  return true
}
